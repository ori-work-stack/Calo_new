  ...
  static async getNutritionStatistics(
    ...
    try {
      ...
      const userStats: UserStats = {
        ...
        level: user.level || 1,
        totalWaterGoals: waterIntakes.filter((w) => w.cups_consumed >= 8)
          .length,
      ...
      const streakMetrics = this.calculateStreakMetrics(
        meals,
        waterIntakes,
        userStats
      );
      const wellbeingMetrics = this.calculateWellbeingMetrics(
        meals,
        waterIntakes
      );
      ...
      const achievementData = this.formatAchievements(
        allAchievements,
        userAchievements,
        userStats
      );
